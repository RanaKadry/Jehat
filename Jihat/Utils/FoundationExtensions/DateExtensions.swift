...
extension Date {
    ...
    static func localizedDate(date: String) -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.locale = Locale(identifier: LocalizationHelper.getCurrentLanguage())
        dateFormatter.dateFormat = DateFormates.daySlashMonthSlashYear.rawValue
        guard let convertedDate = dateFormatter.date(from: date) else { return "" }
        return dateFormatter.string(from: convertedDate)
    }
    
    static func localizedTime(time: String) -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = DateFormates.time.rawValue
        dateFormatter.locale = Locale(identifier: LocalizationHelper.getCurrentLanguage())
        dateFormatter.timeZone = TimeZone.current
        guard let convertedDate = dateFormatter.date(from: time) else { return "" }
        return dateFormatter.string(from: convertedDate)
    }
    
    static func getDateDay(from date: Date) -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.locale = Locale(identifier: LocalizationHelper.getCurrentLanguage())

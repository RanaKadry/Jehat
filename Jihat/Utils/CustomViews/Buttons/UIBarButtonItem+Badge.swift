...
extension CAShapeLayer {
    func drawRoundedRect(rect: CGRect, andColor color: UIColor, filled: Bool) {
        fillColor = filled ? color.cgColor : UIColor.white.cgColor
        strokeColor = color.cgColor
        path = UIBezierPath(roundedRect: rect, cornerRadius: rect.width / 2).cgPath
    }
}
...
extension UIBarButtonItem {
    private var badgeLayer: CAShapeLayer? {
        if let bAnyObject: AnyObject = objc_getAssociatedObject(self, &handle) as AnyObject? {
            return bAnyObject as? CAShapeLayer
        } else {
            return nil
        }
    }
    ...
    private func addBadge(text: String, withOffset offset: CGPoint = CGPoint.zero, andColor color: UIColor = UIColor.red, andFilled filled: Bool = true, andFontSize fontSize: CGFloat = 11) {
        ...
        let xPosition = view.frame.width - width + offset.x

        let badgeFrame = CGRect(origin: CGPoint(x: xPosition, y: offset.y), size: CGSize(width: width, height: height))

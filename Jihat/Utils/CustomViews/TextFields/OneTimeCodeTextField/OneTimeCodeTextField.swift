...
class OneTimeCodeTextField: UITextField {
    ...
    @IBInspectable var selectedDigitTextColor: UIColor?
...
//        becomeFirstResponder()
    ...
    private func textDidChange(_ sender: UITextField) {
        ...
        for iDigit in 0 ..< digitLabels.count {
            let currentLabel = digitLabels[iDigit]
            
            if iDigit < text.count {
                let index = text.index(text.startIndex, offsetBy: iDigit)
        ...
        if isFirstResponder {
            digitLabels.forEach {
                if $0.text != defaultCharacter {
                    $0.textColor = selectedDigitTextColor ?? .red
                } else {
                    $0.textColor = digitTextColor
                }
            }

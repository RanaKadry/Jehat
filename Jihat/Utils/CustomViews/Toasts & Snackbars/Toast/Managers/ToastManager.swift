...
class ToastManager {
    ...
    func showError(message: String, view: UIView, completion: (() -> Void)?) {
        let errorHeader: ToastView? = ToastView()
        errorHeaders.forEach({
            hideBanner(errorHeader: $0, completion: completion)
        })
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3) { [weak self] in
            self?.hideBanner(errorHeader: errorHeader, completion: completion)
        }
    }
    ...
    private func hideBanner(errorHeader: ToastView?, completion: (() -> Void)?) {

...
final class AudioRecorderManager: NSObject {
	let audioFileNamePrefix = "com.Jehat-"
	...
	func askPermission(completion: ((Bool) -> Void)? = nil) {
		AVAudioSession.sharedInstance().requestRecordPermission { [weak self] granted in
			...
			completion?(granted)
			print("Audio Recorder granted permission:", granted)
		}
	}
	...
	fileprivate func internalStartRecording(with audioVisualizationTimeInterval: TimeInterval) throws -> URL {
		...
		let recordSettings = [
			AVFormatIDKey: Int(kAudioFormatMPEG4AAC), // NSNumber(value:kAudioFormatAppleLossless)
		...
		guard let path = URL.documentsPath(forFileName: self.audioFileNamePrefix + NSUUID().uuidString + ".m4a") else {
			print("Incorrect path for new audio file")
			throw AudioErrorType.audioFileWrongPath
		}
        print("NEW AUDIO FILE PATH:", path.absoluteString)
        ...
        self.currentRecordPath = path.absoluteURL
		return path
	}

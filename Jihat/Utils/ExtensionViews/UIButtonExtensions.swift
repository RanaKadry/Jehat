...
extension UIButton {
    ...
    @IBInspectable var flipRightToLeft: Bool {
        set {
            if LocalizationHelper.isArabic() && newValue == true {
...
extension UIButton {
    ...
    func setupButtonWithImage(englishTitleEdgeInsets: UIEdgeInsets = .init(top: 0, left: 10, bottom: 0, right: -10), arbabicTitleEdgeInsets: UIEdgeInsets = .init(top: 0, left: -10, bottom: 0, right: 10)) {
        if !LocalizationHelper.isArabic() {
            titleEdgeInsets = englishTitleEdgeInsets
            semanticContentAttribute = .forceLeftToRight
            contentHorizontalAlignment = .leading
        } else {
            titleEdgeInsets = arbabicTitleEdgeInsets
